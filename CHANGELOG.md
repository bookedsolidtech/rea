...
- a406c01: Global-first is now THE model. `rea init` / `rea upgrade` no longer add a local
...
- 1c25bb8: Add an opt-in **global rea CLI resolution tier** so rea's hooks can govern a
...
- 8a980bb: feat(0.49.0): rea init self-pin + bash-gate bootstrap allowlist
...
- 5649a66: 0.46.0 — audit observability expansion + profiling fidelity fix.
...
- 53124b6: 0.45.0 — hook hot-path profiling pass + 0.44.0 round-2 polish.
...
- 613cb2a: `rea init` clack flow polish + round-7 P3 — consumer-facing first-impression refresh.
...
- f0c7d53: Consumer-UX bundle: `rea upgrade --check` + `rea audit summary` + README refresh.
...
- f68bcae: feat(0.39.0): policy-reader doctor checks — surface 4-tier ladder reachability
...
- 4516d70: Extract `_lib/shim-runtime.sh` — shared Node-binary shim runtime.
...
- 657e935: Unified shim policy-fallback (4-tier graceful-degradation reader).
...
- e09459a: Node-binary hook playbook — final 4 paired scanner-backed ports
...
- adb347b: feat(0.34.0): Node-binary hook playbook — 3 tier-2 medium-complexity ports
...
- 527df24: feat(0.33.0): Node-binary hook playbook — 4 tier-1 advisory/single-purpose ports
...
- 75abfe3: 0.32.0 — Node-binary hook playbook (Phase 0 + Phase 1 + Phase 3).
...
-r '.tool_input.command // ""'` shape every bash hook repeats, with
  ...
  **Phase 1 — three pilot hook ports.** The bash bodies move into
  ...
  Each pilot ships: - Pre-0.32.0 baseline preserved at
  `__tests__/hooks/parity/baselines/<name>.sh.pre-0.32.0`. - 4 new `rea hook <name>` CLI subcommands (only `policy-get` and
  `scan-bash` / `codex-review` shipped pre-0.32.0). - 11 bash↔node parity tests under
  ...
  **Phase 3 — `.husky/prepare-commit-msg.d/*` extension surface.** The
  ...
  idempotency hit, skip*merge match, missing policy, REA_SKIP*
...
- a29fcf2: feat(0.31.0): delegation-telemetry completion — the nudge, roster discovery, audit-specialists flags

  0.29.0 shipped the delegation-telemetry _observability_ layer (the
  `Agent|Skill` PreToolUse capture hook + `rea audit specialists`
  reader) — it could _see_ delegation patterns but said nothing about
  them. 0.31.0 closes the loop with the _nudge_ plus four supporting
  surfaces.
...
delegation-advisory` CLI.** A new PostToolUse hook on matcher
  ...
  recorded zero _real_ delegation signals, it prints a one-time stderr
  ...
  **4. Live `.claude/agents/` roster discovery.** The nudge's "did this
  session delegate to a _real_ specialist" predicate discovers the
...
- a09f94f: fix(0.30.1): round-5 P2 hardening sweep — settings strict schema, doctor hooks-dir resolution, control-char name guard
...
- a0bab9e: # 0.30.0 — attribution augmenter, Class M settings.json schema, 0.29.0 drift closure
...
- a764af1: feat(0.29.0): delegation-telemetry MVP — observe `Agent` / `Skill` dispatches
...
- c7d9ebb: fix(0.28.2): close `/./` Write-tier bypass + npm CDN retry + orchestrator self-review
...
- b79ae5d: fix(review): close defect-V agent-blocking finding-text gap
...
- 07c5eb0: `rea verify-claim` CLI plus five surgical helix-closure items.
...
- 756a6b9: Wave 3 specialist agents + figma-dx-specialist + helix-031 shellcheck fix + codex-review invocation pattern.
...
- d3a3520: Local-first delegation enforcement (CTO directive 2026-05-05, applies to ALL rea work — OSS + enterprise).
...
- 5f863c6: Roster expansion Wave 2 — 3 Architects (CTO eval recommendation).

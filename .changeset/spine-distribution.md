...
process-spine skills automatically. The `spine/` payload installs to a dedicated
`.claude/skills/` target via a single canonical DirMapping, so it version-pins,
...
`.claude/commands/`, flags the quarterly prune; never fails the exit code).

...
# artifact_gates.g3_review.mode; enforce > shadow > off). REA_ROOT is the local
# worktree root resolved above; policy is per-branch/worktree, so the LOCAL
# policy governs. Dependency-free (awk only), BSD/GNU/bash-3.2 safe, single
# pass. Dispositions (round-54 tri-state — shadow is OBSERVE-ONLY, never blocks):
#   - policy ABSENT                        → \`off\`  (no gate → ALLOW)
...
# before comparing, so \`enforce\`, \`"enforce"\`, \`'enforce'\`, and \`enforce # note\`
# all match. Known limit: no full YAML parse — a structurally broken policy that
# our line scan reads as no-mode maps to \`off\` (ALLOW).

...
export function registerHookCommand(program: Command): void {
  ...
  hook
    ...
    .description(
      'PostToolUse Bash billing→HALT reflex (0.51.0, spend-governance E1 seed). Reads a Claude Code PostToolUse Bash payload from stdin and scans the ERROR channel (tool_response.stderr, plus stdout only when the call FAILED) — NOT the command text or successful stdout, so `cat`/`rg` on billing docs never self-freeze; when a BILLING-CLASS signature (spending cap / prepayment credits depleted / payment required / insufficient balance — DISTINCT from a retryable 429/rate-limit) matches, writes `.rea/HALT` (reusing the kill-switch every middleware + hook respects) per `policy.spend_governance.billing_error_response`: halt (default) → HALT + banner + exit 2; warn → banner + exit 2, no HALT; off / disabled / absent block → no-op. Malformed payload is fail-SAFE (exit 0, no freeze); the shim fails CLOSED only when a billing keyword is present with no CLI. Introduced after INCIDENT-2026-07-04 (denial-of-wallet).',
    )
    .action(async () => {

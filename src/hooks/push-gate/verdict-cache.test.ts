...
describe('verdict-cache — write + lookup roundtrip', () => {
  it('lookup returns hit for fresh entry within TTL', () => {
    writeVerdict(baseDir, 'sha-abc', FRESH_ENTRY());
  ...
  it('lookup returns miss for unknown SHA in existing cache', () => {
    writeVerdict(baseDir, 'sha-abc', FRESH_ENTRY());
    const r = lookupVerdict(baseDir, 'sha-xyz');
    expect(r.hit).toBe(false);
  });

  it('writes the cache file with the documented schema version', () => {
    writeVerdict(baseDir, 'sha-abc', FRESH_ENTRY());
  ...
  it('preserves prior entries on subsequent writes', () => {
    writeVerdict(baseDir, 'sha-abc', FRESH_ENTRY({ verdict: 'pass' }));
    writeVerdict(baseDir, 'sha-def', FRESH_ENTRY({ verdict: 'concerns' }));
...
describe('verdict-cache — clear + prune', () => {
  it('clearVerdict removes one entry', () => {
    writeVerdict(baseDir, 'sha-a', FRESH_ENTRY());
    writeVerdict(baseDir, 'sha-b', FRESH_ENTRY());
    expect(clearVerdict(baseDir, 'sha-a')).toBe(true);
    expect(lookupVerdict(baseDir, 'sha-a').hit).toBe(false);
    expect(lookupVerdict(baseDir, 'sha-b').hit).toBe(true);
  });

  it('clearVerdict returns false on unknown SHA', () => {
    writeVerdict(baseDir, 'sha-a', FRESH_ENTRY());
    expect(clearVerdict(baseDir, 'sha-unknown')).toBe(false);
  });

  it('clearAll empties all entries and returns the count', () => {
    writeVerdict(baseDir, 'sha-a', FRESH_ENTRY());
    writeVerdict(baseDir, 'sha-b', FRESH_ENTRY());
    expect(clearAll(baseDir)).toBe(2);
    expect(Object.keys(listEntries(baseDir))).toHaveLength(0);
  });

  it('pruneOlderThan removes entries older than the cutoff', () => {
    writeVerdict(baseDir, 'sha-old', FRESH_ENTRY({ reviewed_at: '2026-01-01T00:00:00.000Z' }));
    writeVerdict(baseDir, 'sha-new', FRESH_ENTRY({ reviewed_at: new Date().toISOString() }));
    const removed = pruneOlderThan(baseDir, 7 * 24 * 60 * 60 * 1_000); // 7 days
...
describe('verdict-cache — corruption resilience', () => {
  ...
  it('lookup returns miss when reviewed_at is unparseable', () => {
    writeVerdict(baseDir, 'sha-a', FRESH_ENTRY({ reviewed_at: 'not-a-date' }));
    expect(lookupVerdict(baseDir, 'sha-a').hit).toBe(false);
  });
});

describe('verdict-cache — atomic write', () => {
  it('write is atomic via tmp-file + rename (no partial file under racing reads)', () => {
    writeVerdict(baseDir, 'sha-a', FRESH_ENTRY());

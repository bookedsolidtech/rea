...
> **Numbering note:** the OpenRouter lane (merged via PR #210) originally also claimed §5.25; it is now **§5.26/§5.26a** (renumbered post-merge). The two sections are independent; the collision was purely ordinal.
...
- **Estimate-vs-rated ledger confusion.** The real-time "estimate" ledger and the vendor-settled "rated" ledger diverge by hours and by multiples. rea cannot and does not see rated dollars — those arrive long after the session. What rea *can* govern is **request volume as a spend proxy**: the audit log records *attempts*, not vendor-settled spend. This is an honest, load-bearing limitation. rea's own OpenRouter lane (§5.26) is itself such a metered endpoint and is in scope for this proxy, not for dollar accounting.

**0.51.0 closure (E1 seed slice).** The first structural anchor is the `billing-cap-halt.sh` PostToolUse Bash hook (`src/hooks/billing-cap-halt/index.ts`), driven by the new `spend_governance` policy block (`src/policy/types.ts`, `src/policy/loader.ts`, `.strict()`). It scans a FAILED command's **stderr** for a **provider-specific billing-class** signature — a TERMINAL, non-retryable spend error — and, per `spend_governance.billing_error_response`, acts. The **SEED DEFAULT is `warn`** (banner + exit 0, NO freeze), NOT `halt`: because the hook fires on every Bash PostToolUse with no metered-endpoint scoping yet, a phrase-only global `halt` would false-freeze finance/payments-domain repos that emit the same phrases (codex 0.51.0 round-12 P1). `halt` (writes `.rea/HALT`, exit 2) is an explicit opt-in and becomes the default once the E1 metered-endpoint registry supplies the provider discriminator. To further bound false positives (a self-inflicted denial-of-service is its own harm), it scans the **`tool_response.stderr` channel ONLY, and ONLY when the command failed** — NEVER the command text, and NEVER stdout even on a non-zero exit. Otherwise benign work would freeze the session: `cat THREAT_MODEL.md` / `rg "spending cap" .` print the watched phrases to stdout, and `grep -R "spending cap" docs missing_dir` exits non-zero on the missing path while printing real doc matches to stdout (codex 0.51.0 round-1 P1/P2, round-4 P1). A billing error printed only to a failed command's stdout is an accepted gap for this coarse backstop; full-output scanning returns with the E1 metered-endpoint registry, where a known metered host justifies it. This deliberately reuses the existing kill-switch: every middleware and hook already respects `.rea/HALT`, so "billing error ⇒ stop everything, no retry" needs no new enforcement primitive — the automated hook is the analog of `rea freeze`.
...
### 5.26 OpenRouter OSS review provider — outbound diff trust boundary (0.50.x)

**New boundary.** The `openrouter` review provider (`policy.review.provider: openrouter | both`) sends the working-tree diff off-machine to `openrouter.ai` so an open-weight model (`gpt-oss-120b`) can run an adversarial review in place of, or in parallel with, OpenAI Codex. This is the same *class* of exposure rea already accepts on the Codex→OpenAI lane (a diff leaves the machine for review), but it is a distinct endpoint and account, so the controls are made explicit. Codex stays the default; this lane is opt-in.

**Threat:** Privileged or evidentiary content (legal matter under the `strawn-legal` tree, secret-bearing files, anything in `blocked_paths` or `.rea/`) is sent to a third-party inference endpoint that — on the wrong backend or the `:free` tier — could log or train on the prompt.

**Mitigations (fail-closed, layered):**

- **Path-guard is the PRIMARY control, and it runs BEFORE any diff bytes are assembled into a request body.** The changed-path set is enumerated from git plumbing ONLY — never by parsing diff text. **Guard==diff invariant (load-bearing):** the guard's evaluated path set is byte-for-byte identical to the set the outbound diff (`assembleDiff`) actually sends: `git diff <base>...HEAD` (committed) ∪ `git diff HEAD` (tracked working-tree + index changes vs HEAD) — i.e. TRACKED changes ONLY. Untracked files are never in the outbound payload, so they are never in the guard set either; a path can never leave the guard set while the diff still includes it. (An earlier draft enumerated untracked files and then dropped `.rea/*` from the set with a post-filter — that combination both false-refused every real review and, for a *tracked* `.rea/*` change, let the file stay in the diff while leaving the guard; both are fixed by aligning the two sets on the same `git diff` shape.) The refuse-set is `blocked_paths` ∪ the always-on `.rea/` trust root ∪ NON-OVERRIDABLE compiled constants (the `strawn-legal` tree and any `.secret.` file glob) ∪ `path_overrides` whose provider is `codex`/`refuse`. Policy may ADD refusals; it can NEVER subtract the evidentiary constants. Each path classifies ALLOW / REFUSE / UNCERTAIN, and **UNCERTAIN ≡ REFUSE**; if ANY changed path is REFUSE or UNCERTAIN the WHOLE diff refuses (no per-file split). UNCERTAIN sources: malformed URL-escape, deeply-encoded separator, a `..` traversal segment, a glob-compile failure, a realpath that resolves outside the repo root or into a refuse-set directory (symlink-escape), a git-enumeration error/timeout, or a malformed `path_override`. The matchers are reused from the production `blocked-paths` middleware and the hook-tier `path-normalize` helper; the evidentiary set adds a `**`-aware glob matcher. **The hard invariant: `strawn-legal` never routes to the external OSS lane, even if a misconfigured `path_override` points that glob at `openrouter`** — the evidentiary check runs before the override check, so the override cannot win. **A TRACKED `.rea/*` change refuses the external lane** (governance config never leaves the box); an UNTRACKED `.rea/policy.yaml` — the real-consumer case — is not sent by the diff and so produces no spurious refusal. On refusal the lane falls back to its declared local lane (default `codex`); when the fallback is unavailable in enforced mode the run exits 2, and a `rea.local_review.refused_external` audit record (carrying only the matched RULE, never the raw path, and a changed-path COUNT, never the paths or diff) is written BEFORE the fallback.

- **Redaction is defense-in-depth, at a SINGLE chokepoint AFTER payload assembly and BEFORE `JSON.stringify` + `fetch`.** The full outbound body runs through the shared default secret-pattern redactor (`compileDefaultSecretPatterns` + `redactSecrets`) with a bounded per-call timeout. **What egresses (disclosed):** the raw patch (diff CONTENT) AND a short commit-log header — `git log --oneline` over the reviewed range, i.e. commit SUBJECTS and BRANCH names, which can carry PII (ticket URLs, customer names). Both pass through the same redaction chokepoint. A governance layer never relies on a regex redactor as the *only* thing between privileged data and an external endpoint — the path-guard above is the actual control; redaction catches a secret that slipped into a non-sensitive file. On redactor timeout (the `REDACT_TIMEOUT_SENTINEL`) the send is ABORTED and the lane falls back to codex (`refusal_class: redact-timeout`) — we never send a body the scanner failed to clear. The assembled diff is byte-capped at `max_diff_bytes` (default ~1.5 MB); over-cap falls back to codex (`refusal_class: diff-too-large`), never truncate-and-send.

- **Commit-aware chunking preserves every invariant (guard-whole + redaction-per-unit + fail-closed).** `gpt-oss-120b` rejects a diff larger than its context window (~350KB) BEFORE inference. **Granularity semantics (codex round-11 P1):** `review_granularity: auto` (default) sends the WHOLE diff when it fits the context budget, else ESCALATES TO CODEX (`diff-too-large`) — `auto` NEVER silently adopts per-commit semantics, because reviewing each intermediate commit (`C^..C`) is NOT equivalent to reviewing the net `base...HEAD` (a problem introduced in commit A and FIXED in commit B would be flagged against the clean final diff). `per-commit` is an EXPLICIT opt-in for the "review each commit as you make it" practice — each commit is reviewed independently on its own patch, and a later fix-up does NOT suppress an earlier commit's finding (intentional, and distinct from a net-diff gate). `whole` never splits. When `per-commit` is selected, chunking is STRICTLY downstream of the path-guard: the guard still runs ONCE on the whole changed-path set and is byte-for-byte unchanged — splitting only partitions what is SENT, post-approval. The **guard==send invariant holds across the split**: the union of all units' changed paths (each commit's own `C^..C` patch ∪ the trailing `working-tree` unit's `git diff HEAD`) equals exactly the whole-diff changed-path set the guard evaluated (`base..HEAD` ∪ working-tree-vs-HEAD) — a path can never enter a sent unit without having passed the whole-diff guard, and the guard is never re-run per unit. Each unit's content is REDACTED independently through the SAME chokepoint before its send (no unredacted bytes ever leave on any unit). Every unit is fail-closed: a unit whose redacted content exceeds the context budget OR `max_diff_bytes` (a single commit too big to split further) escalates the WHOLE review to codex (`diff-too-large`) — an over-budget unit is NEVER sent; any unit's non-`ok` attempt (malformed-after-repair, backend-pin violation, timeout, HTTP exhaustion, unauthorized) is handled EXACTLY as the whole-diff failure path (unauthorized surfaces; the rest fall back to codex with that unit's refusal class) — a result set with a silently-dropped unit is never merged. Every per-commit git invocation carries the same `--no-ext-diff --no-textconv` + `SAFE_DIFF_CONFIG` + scrubbed-env hardening and goes through `diffStdoutOrThrow` (a git error throws `DiffAssemblyError` → codex, never a silently-empty unit). When all units succeed they merge into ONE synthetic outcome (verdict = max severity, findings = union capped at `MAX_FINDINGS` — **the cap keeps the HIGHEST-severity findings (codex round-11 P2): a stable severity sort runs before the slice so a later unit's P1/P2 that drove the merged verdict is never dropped in favour of an earlier unit's P3**, usage summed) feeding the existing finalization unchanged — one audit record, one outcome, downstream-indistinguishable from a whole-diff review.

- **Data-policy is REQUESTED, not VERIFIED; backend identity is verified against the response.** `data_collection: 'deny'` is a routing constraint enforced on OpenRouter's side, NOT a guarantee rea can confirm — OpenRouter returns no per-response no-training acknowledgment. The record states `data_policy_requested` (what we asked) and a derived `data_policy_enforced` that is `pin-verified` ONLY when a non-empty `backend_pin` is set AND `served_by ∈ pin` (the operator's asserted vetted-no-train allowlist); otherwise `routing-requested`, which is the default `backend_pin: []` case and is NOT a verified guarantee. `:free` is rejected at policy load. served_by is char-validated and (when pinned) verified ∈ pin; outside-pin → discard+codex fallback (backend-pin-violation). `pin-verified` asserts only that served_by is in the operator's declared pin — rea cannot prove a backend's training behavior; that vetting is the operator's responsibility.

- **Key handling.** The API key is resolved ENV-FIRST (`process.env.OPENROUTER_API_KEY`), then from a rea-managed credentials file, then NONE — see §5.26a for the managed-file security model. Wherever it comes from, the key is used in an `Authorization: Bearer` header via in-process `fetch` (no `curl` subprocess), and is NEVER logged, audited, placed in telemetry, or echoed by `rea doctor`. Doctor reports key PRESENCE (a bool) and its SOURCE (`env` / `config file`) and, when an operator runs the optional reachability probe, a reachability STATE (`reachable` / `unreachable` / `unauthorized`) — never the value, prefix, or length.

- **No shipped test seam; invalid policy fails CLOSED.** There is intentionally NO `REA_OPENROUTER_FIXTURE` (or any env-var fixture) on the shipped provider: a test seam reachable from production `process.env` would let any environment with that var set mint a canonical `rea.local_review` PASS from attacker-controlled local JSON — a trust bypass. Unit tests inject a mocked transport via the in-process `OpenRouterTransport` / `RunReviewDeps.executeOpenRouterReview?` seam (never production env); the black-box harness drives the REAL `fetch` transport against a `http://127.0.0.1:<port>` localhost responder. The `base_url` validator is HOST-PINNED, not merely scheme-pinned: an https URL is accepted ONLY for host `openrouter.ai` (or a subdomain), so `https://evil.example/api/v1` is rejected at policy LOAD (it would otherwise receive the full diff + commit log). The plaintext-http exception is NARROW — the IPv4 loopback block `127.0.0.0/8` (octets bounded 0-255), `[::1]`, or exactly `localhost`, with an optional 1-65535 port; it is anchored so `127.0.0.1.evil.com` / `localhost.evil.com` / `openrouter.ai.evil.com` / `evilopenrouter.ai` and any public/LAN host, out-of-range octet, or out-of-range port are rejected. Separately, a `.rea/policy.yaml` that EXISTS but is malformed / schema-invalid makes the external lane FAIL CLOSED: the provider refuses (falls back to codex, `refusal_class: invalid-policy`) rather than proceeding with permissive empty `blocked_paths` / `path_overrides` that would bypass the `:free` / HTTPS / path-override safeguards — precisely when an operator is mid-edit on `review.providers.openrouter`. A genuinely MISSING policy is distinct: defaults are safe there (the evidentiary constants still guard, the key check still gates).

- **Degradation never hangs and never silently passes.** HTTP 429 / 5xx → bounded exponential backoff → fallback backend within the pin → codex. Malformed / schema-violating output → ONE repair retry → codex escalation; if codex is unavailable the run yields `verdict: error` (exit 2), the designed-for non-execution outcome — never a silent pass. In `provider: both` the OpenRouter outcome is written under the distinct informational `rea.local_review.shadow` tool name and is NEVER preflight coverage, so a gpt-oss `pass` can never launder a codex `blocking`.

- **Response is untrusted INGRESS — one sanitizer chokepoint before any sink.** The model's response is attacker-influenceable data that lands in the hash-chained `.rea/audit.jsonl`, `.rea/last-review.json`, stdout, and `.rea/metrics.jsonl` — and `appendAuditRecord` does NOT redact (it only validates JSON round-trip). So the adapter (`adaptOpenRouterResponse` / `extractServedBy`) is the SINGLE ingress chokepoint and scrubs every model string before it reaches any sink: finding `title`/`body` are control-char-stripped → secret-REDACTED (the same `redactSecrets` the codex path uses — the model echoes diff content, including possible secrets, verbatim into its findings) → byte-capped; the findings array is capped (`MAX_FINDINGS = 50`, excess dropped + a truncation marker recorded) and each body capped (~8 KB), so a hostile/buggy endpoint returning 100k findings cannot bloat the audit log or drive O(n·m) parity loops; `served_by` is char-validated (`^[a-zA-Z0-9._-]{1,64}$` after a control-char strip) and DROPPED on violation, so a 10 KB / control-char / `"codex"`-forgery value never reaches audit or metrics; token counts from the response `usage` block are clamped (forged absurd values can't write nonsense cost to metrics). The verdict is derived from the (sanitized) findings, never the model's self-verdict alone (§ above / round-5).

- **Outbound diff is byte-exact; the transport is bounded.** Both the guard's path enumeration and the payload assembly run `git diff` with `--no-ext-diff --no-textconv -c diff.external= -c core.attributesFile=/dev/null -c core.quotePath=false` and a scrubbed env (no `GIT_EXTERNAL_DIFF`/`GIT_DIFF_OPTS`), so the bytes are the RAW patch — never an external/textconv helper's stdout (which could emit arbitrary checkout bytes past the path-guard, or hang). Path enumeration is NUL-delimited (`--name-only -z`, split on `\0` only) so the guard sees REAL byte-exact paths — a secret file with a NON-ASCII or newline-containing name is classified on its real path, not a mangled `core.quotePath` quoted form, so it cannot slip the guard while being sent verbatim (the guard==diff invariant holds for non-ASCII / newline names). An operator `path_override` glob that fails to compile is UNCERTAIN ≡ refuse (fail-CLOSED), not a silent no-op. The production `fetch` sets `redirect: 'error'` (a `base_url` that 302s to a non-pinned host is refused, not silently followed before the pin check) and reads the response body with a hard byte cap (~4 MB) under the per-attempt abort signal (no unbounded `text()` → `JSON.parse`; a slow-dribble 200 body cannot stream past the timeout).

**Trust boundary:** the diff content crosses from the trusted local process to an untrusted third-party endpoint, and the response crosses back from the untrusted endpoint into the hash-chained audit log. The path-guard decides (over the byte-exact changed-PATH set, from git plumbing) WHETHER the send is permitted; the redactor scrubs the bytes that do go; the backend-pin + data-policy verification constrains WHERE they land and HOW they may be used; the response-ingress sanitizer scrubs everything that comes back before it touches a sink.

**Residual risk:** a secret embedded in a file that is NOT in the refuse-set still reaches the redactor as the last line of defense — bounded by the same secret-pattern catalog the Codex→OpenAI lane already depends on, no better and no worse. A novel secret shape no pattern catches could transit on a non-sensitive path; this is the identical residual the existing redaction surface carries (§5.5), now applied to a second outbound lane. The path-guard removes the *evidentiary/sensitive-path* class entirely (fail-closed), which is the class that matters most for this provider's intended consumers. The no-training posture on the OpenRouter lane is requested, not proven. A consumer on the default `backend_pin: []` trusts OpenRouter's routing enforcement, not a rea-verified guarantee. Consumers needing the stronger posture should populate `backend_pin` with independently vetted no-train backends; the evidentiary/strawn-legal class never takes this lane (path-guard, fail-closed).

Ref: `src/cli/review-provider.ts`, `src/cli/review-openrouter.ts` (response-ingress sanitizer, `--no-ext-diff`/`--no-textconv`/`-z`/`core.quotePath=false`, body cap + `redirect:error`, token clamp, `enumerateReviewUnits`/`mergeUnitAttempts` commit-aware chunking, `OPENROUTER_CONTEXT_BUDGET_BYTES`), `src/cli/review-pathguard.ts` (NUL-delimited byte-exact enumeration, tri-state `matchDoubleStarGlob` fail-closed), `src/cli/review-shadow.ts` (`startShadowExecution` concurrent shadow kickoff), `src/gateway/middleware/redact.ts` (`sanitizeInput` reused at the ingress chokepoint), `src/audit/local-review-event.ts` (`rea.local_review.shadow`, `rea.local_review.refused_external`), `src/policy/{types,loader}.ts` (`review.provider`, `review.providers.openrouter.*`, `review_granularity`, `:free` guard, narrow loopback-http `base_url` exception), `src/cli/review-openrouter.test.ts` (ingress sanitizer, guard==diff, body cap/redirect, no-env-fixture, invalid-policy-fail-closed), `src/cli/review-openrouter.commit-aware.test.ts` (per-commit enumeration, merge, fail-closed over-budget/malformed unit), `src/cli/review.round8.test.ts` (invalid-both fail-closed, shadow concurrency), `src/cli/review-pathguard.test.ts` (no-ext-diff, non-ASCII/newline byte-exact paths, P2-5 fail-closed, `.rea/` no-leak), `src/policy/loader.openrouter.test.ts` (base_url loopback narrowness, `review_granularity` enum).

### 5.26a OpenRouter key source — managed credentials file (0.50.x)

**Why this exists.** The openrouter lane (§5.26) needs an `OPENROUTER_API_KEY`. Requiring every operator to hand-edit `~/.zshenv` is not a turnkey setup — it is shell-specific, brittle, and invisible to `rea doctor`. `rea config set-key openrouter` gives rea a governed place to keep the key. This subsection is the trust model for THAT store; §5.26's outbound boundary is unchanged by where the key is read from.

**No OS keychain — deliberate.** The OS keychain unlock is a GUI modal. rea runs non-interactively (CI, agents, the pre-push gate); a modal that blocks a governance gate makes the gate flaky. The keychain was evaluated and REJECTED for this reason. The store is a flat managed file ONLY: `${XDG_CONFIG_HOME:-~/.config}/rea/credentials`, dir `0700`, file `0600`, one `KEY=value` per line, written atomically (temp + `rename`).

**Resolution precedence is ENV-FIRST.** `resolveOpenRouterKey` returns the env var if present and non-empty, else the file value, else none. Env-first is intentional: a per-project or CI run overrides the global default with a per-invocation key, and every pre-existing `export OPENROUTER_API_KEY=…` setup keeps working byte-for-byte. The file is the convenience default, never an override of an explicit env decision.

**Reader is FAIL-CLOSED — file AND parent dir.** A symlinked PARENT directory (`…/rea` — could redirect the read/write outside `~/.config/rea` even when the file itself looks fine), a non-directory at that path, a foreign-owned or group/other-WRITABLE parent dir, a symlinked credentials file, a non-regular file, group/other-accessible file permissions (`mode & 0o077 != 0`), or foreign file ownership (`st.uid != getuid()`) all REFUSE: the resolver returns `source: 'none'` with a `refusal` reason, never a key. The parent-dir gate runs BEFORE the file is touched on both the read and the write path, so a symlinked directory can never redirect a key write either. A refused file degrades the provider to *unavailable* (→ codex fallback or policy refusal per §5.26) — it never silently mints trust from an attacker-staged or world-readable file. The perm/owner gate is POSIX-only (skipped where `process.getuid` is undefined, e.g. Windows, where mode bits are not meaningful). `rea config set-key` likewise refuses to overwrite a creds file that already fails the gate (it does not clobber a symlink — `rename` replaces the path, not a link's target).

**Key value never transits argv.** `rea config set-key` reads the value from a masked `@clack/prompts` password prompt (interactive) or `--stdin` (CI) — there is NO `--value`/positional flag, so the secret never lands in shell history, `ps`, or the audit log. `get-key`/`list` print only a masked fingerprint (last 4 chars) and the SOURCE; they never echo the body. `rea doctor` surfaces presence + source + (on refusal) the refusal reason, never the value.

**Test isolation.** The credentials dir is derived ONLY from the passed env bag (`XDG_CONFIG_HOME` then `HOME`); a curated env with neither set resolves to "no path" and the filesystem is never touched. Production passes `process.env` (has `HOME`); unit tests pass minimal envs and stay hermetic — they cannot read a developer's real `~/.config/rea/credentials`, and a developer with a real key cannot perturb the suite.

**Residual risk:** the file is plaintext at `0600`, readable by the operator's own uid and root — the same posture as `~/.aws/credentials`, `~/.npmrc`, or a `.env`. rea does not add encryption-at-rest; an attacker who already has the operator's uid (or root) has the key regardless of store. The control rea adds over an ad-hoc dotfile is the fail-closed gate (no symlink/world-readable/foreign-owned read), env-first override, and `doctor` visibility. The key still grants spend on the operator's OpenRouter account; revocation is the operator's responsibility on compromise.

Ref: `src/cli/openrouter-key-source.ts` (resolver, fail-closed reader, atomic writer), `src/cli/config-key.ts` (`set-key`/`get-key`/`unset-key`/`list`, stdin/masked-prompt only), `src/cli/openrouter-key-source.test.ts` (symlink/perms/owner refusal, env-first precedence, test-isolation), `src/cli/config-key.test.ts` (stdin ingestion, masked output, env-override note), `src/cli/doctor.ts` (`checkOpenRouterAvailability` source label + refusal surfacing).
...
**Residuals (documented, tracked).** (1) `shim-runtime.sh` re-derives `REA_ROOT` from the payload's `cwd` once stdin is read (guarded on `.rea/` presence and a real JSON parser — jq → python3 → node; regex scraping is deliberately refused since an embedded `"cwd"` inside command text could spoof it), so bash-tier policy reads inside the shim runtime are worktree-correct; the narrower residual is the PRE-stdin window (`check_halt`'s root, covered by the dual probe) plus hosts with none of jq/python3/node and `rea hook policy-get` (no payload) — those stay on `CLAUDE_PROJECT_DIR ?? cwd`. Exposure is advisory-tier: every blocking decision flows through the converted Node hooks, and the CLI-resolution sandbox (`proj`) deliberately stays on the primary checkout, which may be the only one with node_modules installed. (2) Coverage across worktrees rides the `head_sha` fallback of the preflight predicate — the `content_token` form is a working-tree hash and only matches when trees are byte-identical. (3) No auto-migration of pre-0.54.0 per-worktree audit chains (merging two hash chains is not meaningfully possible); `rea doctor` warns on orphaned local state and the operator re-reviews once. (4) Linked worktrees of BARE or `--separate-git-dir` repositories degrade to per-worktree state with a loud advisory: git records no back-pointer from the external metadata dir to the primary checkout (`git worktree list` reports the metadata path itself, `core.worktree` stays unset), so the shared root is genuinely undiscoverable; the main-worktree fallback upgrades automatically if a future git exposes the real checkout path. (5) Rotated audit archives (`audit-YYYYMMDD-HHMMSS.jsonl`) are not in the cross-root protected set — only the live chain, TOFU store, and lock sidecars are; archives are forensic copies whose tampering is detectable via the chain anchor.

...
### 5.25 OpenRouter OSS review provider — outbound diff trust boundary (0.50.x)
...
### 5.25a OpenRouter key source — managed credentials file (0.50.x)

**Why this exists.** The openrouter lane (§5.25) needs an `OPENROUTER_API_KEY`. Requiring every operator to hand-edit `~/.zshenv` is not a turnkey setup — it is shell-specific, brittle, and invisible to `rea doctor`. `rea config set-key openrouter` gives rea a governed place to keep the key. This subsection is the trust model for THAT store; §5.25's outbound boundary is unchanged by where the key is read from.
